        ...
        chrome.runtime.onMessage.addListener(
            function (request, sender, sendResponse) {
                ...
                function endSelection(event) {
                    ...
                    var x1 = Math.min(startX, event.clientX);
                    var y1 = Math.min(startY, event.clientY);
                    var x2 = Math.max(startX, event.clientX);
                    var y2 = Math.max(startY, event.clientY);
                    canvas.width = x2 - x1;
                    canvas.height = y2 - y1;
                    context.drawImage(image, x1, y1, x2 - x1, y2 - y1, 0, 0, x2 - x1, y2 - y1);
                    var imageData = context.getImageData(0, 0, x2 - x1, y2 - y1);
